...
import (
	...
	"github.com/DaviLeonardoSMelo/fc-goapi/internal/entity"
)
...
func (pd *ProductDB) GetProducts() ([]*entity.Product, error) {
	...
	for rows.Next() {
		...
		if err := rows.
			Scan(&product.ID, &product.Name, &product.CategoryID); err != nil {
			return nil, err
		}
...
func (pd *ProductDB) GetProductByCategoryID(categorID string) ([]*entity.Product, error) {
	rows, err := pd.db.Query("SELECT id, name, description, price, category_id, image_url FROM products WHERE category_id = ?",categorID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
